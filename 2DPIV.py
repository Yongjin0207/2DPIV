...
def build_html(
    ...
    atom_html = "".join(
        f'<span class="lgAtomItem"><span class="lgDot" style="background:{c};"></span>'
        f'<span class="lgAtomText">{escape(t)}</span></span>'
        for (t, c) in atom_items
    )
...
function applyTheme() {
  ...
  const dots = document.querySelectorAll(".lgDot");
  if (dots && dots.length >= 2) {
    dots[0].style.background = THEME.posAtom;
    dots[1].style.background = THEME.negAtom;
  }
  ...
  document.querySelectorAll("circle.atom").forEach(el => {
    ...
    if (isPosCharge(res3, name)) el.setAttribute("fill", THEME.posAtom);
...
if (btnSessionDownload) btnSessionDownload.addEventListener("click", ()=>{
  ...
  if (!txt.trim()) return;
  downloadText("2DPIV_session.json", txt);
});
...
def main():
    ...
    ap.add_argument("--out", default="2DPIV.html", help="Output HTML")
